...
export default class Groups extends Manager {
  ...
  getGroupInfoByName(options){
    options = super._objectifyString(options) || {};
    let groupName = options.groupName || "";
    let apiPath = `${BASE_PATH}?name="${groupName}"`;
    options.method = BOX_CONSTANTS.HTTP_VERBS.GET;
    return this.client.makeRequest(apiPath, options);
  }
  ...
  _getMembershipValues(options, values, skipValidation, ignoreModelValues) {
    skipValidation = skipValidation || this.client.skipValidation || false;
    ignoreModelValues = ignoreModelValues || false;

    super._getModel(options, values, skipValidation, ignoreModelValues);
  }
  _getCreateGroupValues(options, values, skipValidation, ignoreModelValues) {
    skipValidation = skipValidation || this.client.skipValidation || false;
    ignoreModelValues = ignoreModelValues || false;

    super._getModel(options, values, skipValidation, ignoreModelValues);
  }
//added function to add user into groups
  createMembership(options){
    options = options || {};
    if (!this.client._simpleMode) {
      const REQUIRED_VALUES = [MODEL_VALUES.USER,MODEL_VALUES.USER_ID,MODEL_VALUES.GROUP, MODEL_VALUES.GROUP_ID, MODEL_VALUES.ROLE];
      let skipValidation = super._setSkipValidation(options);
      let ignoreModelValues = super._setIgnoreModelValues(options);

      this._getMembershipValues(options, REQUIRED_VALUES, skipValidation, ignoreModelValues);
    }
    let apiPath = `${BASE_MEMBERSHIP_PATH}`;
  ...
  createGroup(options){
    ...
    if (!this.client._simpleMode) {
      ...
      let skipValidation = super._setSkipValidation(options);
      let ignoreModelValues = super._setIgnoreModelValues(options);
      const REQUIRED_VALUES = [MODEL_VALUES.NAME];
      this._getCreateGroupValues(options, REQUIRED_VALUES, skipValidation, ignoreModelValues);
    }
    let apiPath = `${BASE_PATH}`;
    options.method = BOX_CONSTANTS.HTTP_VERBS.POST;
    return this.client.makeRequest(apiPath, options);
