...
export default class Manager {
  ...
  _getItemLimit(options) {
    let itemLimit = this._getId(options);
    if (options.itemLimit || options.item_limit) {
      itemLimit = options.itemLimit || options.item_limit;
      (options.itemLimit) ? delete options.itemLimit : delete options.item_limit;
    } else if (options.item && options.item.limit) {
      itemLimit = options.item.limit;
    }
    return itemLimit;
  }
  _getOffset(options) {
    let offset = this._getId(options);
    if (options.offset) {
